...
//unsigned short FAR *work;
{
    ...
    if (max == 0) {                     /* no symbols to code at all */
        this_entry.op = (unsigned char)64;    /* invalid code marker */
        this_entry.bits = (unsigned char)1;
        this_entry.val = (unsigned short)0;
    ...
    for (;;) {
        /* create table entry */
        this_entry.bits = (unsigned char)(len - drop);
        if ((int)(work[sym]) < end) {
            this_entry.op = (unsigned char)0;
            this_entry.val = work[sym];
        }
        else if ((int)(work[sym]) > end) {
            this_entry.op = (unsigned char)(extra[work[sym]]);
            this_entry.val = base[work[sym]];
        }
        else {
            this_entry.op = (unsigned char)(32 + 64);         /* end of block */
            this_entry.val = 0;
        }
    ...
    while (huff != 0) {
        ...
        if (drop != 0 && (huff & mask) != low) {
            ...
            len = root;
            next = *table;
            this_entry.bits = (unsigned char)len;
        }
