...
const unsigned int NUMCACHES=16;
__declspec(thread) void* locMemCache[NUMCACHES] = {0};
__declspec(thread) unsigned int locNumMemCachedItems = 0;

voidpf zcalloc (voidpf opaque, unsigned items, unsigned size)
{
    if (opaque) items += size - size; /* make compiler happy */

	if (locNumMemCachedItems == 0)
